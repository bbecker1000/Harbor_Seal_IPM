...
// ============================================================================
// HARBOR SEAL IPM v3.3
// v3.3: rho_pup added — fraction of year-class pups present at peak molt count
// ============================================================================
...
parameters {
  ...
  // ── Pup molt attendance fraction (v3.3 addition) ──────────────────────────
  // Fraction of the current year-class pups present at haul-out during the
  // peak molt count.  Pups molt first and are on the declining limb of their
  // haul-out curve when adults/juveniles peak; cannot be visually separated
  // from juveniles in the field.  Beta(2,5) prior: mean 0.29, 89% CrI [0.07, 0.55].
...
transformed parameters {
  ...
  matrix<lower=0>[S, T] N_molt_true;   // juv + adult + rho_pup * pup
...

  {
    ...
    for (s in 1:S) {
      ...
      // v3.3: N_molt_true includes rho_pup fraction of pups
      ...
      for (t in 2:T) {
        ...
        // v3.3: include rho_pup fraction of pups in molt count prediction
...
model {
  ...
  p_male_breed ~ beta(200, 800);  //(2, 18) for live model. sensitivity analysis at (50, 950), (100,900), and (20,800)

  // v3.3: rho_pup prior — pups on declining limb of haul-out curve at molt peak
  // Beta(2,5): mean 0.29, mode 0.20, 89% CrI ~ [0.07, 0.55].
  // If sigma_obs_molt decreases relative to v3.2, rho_pup is doing real work.
  rho_pup ~ beta(2, 5);
  ...
  sigma_obs_pup   ~ normal(0.15, 0.02);
  ...
  // ── Vectorized likelihood ─────────────────────────────────────────────────
  {
    ...
    for (i in 1:N_om)
      // N_molt_true already includes rho_pup * N_pup (set in transformed parameters)
...
generated quantities {
  ...
  // ── v3.3: Estimated pup contribution to peak molt counts ──────────────────
  // N_pup_in_molt[s,t] = rho_pup * N_pup[s,t]  — the model-implied number of
  // pups present during the molt survey at site s in year t.
  // N_pup_in_molt_all[t] = sum across sites — useful for checking magnitude.
  matrix[S, T] N_pup_in_molt;
  vector[T]    N_pup_in_molt_all;
  ...
  for (s in 1:S) {
    for (t in 1:T) {
      ...
      // v3.3: pup contribution to molt count
      N_pup_in_molt[s, t] = rho_pup * N_pup[s, t];
    }
  }
  ...
  for (t in 1:T) {
    ...
    N_pup_in_molt_all[t] = rho_pup * sum(col(N_pup, t));
  }
