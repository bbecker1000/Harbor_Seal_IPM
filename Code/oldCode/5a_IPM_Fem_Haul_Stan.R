...
seal_fit_summary <- function(fit, variables) {
  # posterior::summarise_draws() has stable support for named lambda functions;
  # fit$summary() named-argument syntax varies across cmdstanr/posterior versions.
  posterior::summarise_draws(
    fit$draws(variables = variables),
    "mean", "sd",
    q_lo     = ~quantile(.x, CI_LO),
    q_hi     = ~quantile(.x, CI_HI),
    rhat     = posterior::rhat,
...
create_timeseries_plots_v3.2 <- function(fit, sim_data, save=TRUE, prefix="IPM_v3.2") {
  ...
  p_total <- ggplot(tibble(Year=years, mean=colMeans(Ntot),
                           lo=apply(Ntot,2,quantile,CI_LO),
                           hi=apply(Ntot,2,quantile,CI_HI)),
                    aes(x=Year)) +
  ...
  p_phipup  <- ggplot(tibble(x=pup_prob), aes(x=x)) +
    ...
    labs(x="Pup survival probability", y="Density",
         ...
         subtitle=sprintf(paste0("Median = %.3f  (", CI_LABEL, ": %.3f–%.3f)"),
                          median(pup_prob),
  ...
  make_sr <- function(mat) {
    ...
    tibble(Year=years, mean=colMeans(m),
           lo=apply(m,2,quantile,CI_LO), hi=apply(m,2,quantile,CI_HI))
  }
  ...
  mp_df <- tibble(Year=years, mean=colMeans(mp),
                  lo=apply(mp,2,quantile,CI_LO), hi=apply(mp,2,quantile,CI_HI))
  ...
  pull_st <- function(var) {
    ...
    map_dfr(1:S, function(s) map_dfr(1:T, function(t) {
      ...
      tibble(Site=site_names[s], Year=years[t],
             mean=mean(d[,cn]), lo=quantile(d[,cn],CI_LO), hi=quantile(d[,cn],CI_HI))
    }))
  }
...
create_projection_plots_v3.2 <- function(fit, sim_data, save=TRUE, prefix="IPM_v3.2") {
  ...
  proj_df <- map_dfr(1:N_sc, function(sc) {
    ...
    tibble(Scenario=sc_nm[sc], Year=pyrs,
           mean=colMeans(pm), lo=apply(pm,2,quantile,CI_LO), hi=apply(pm,2,quantile,CI_HI))
  })
  
  Ntot <- fit$draws(variables="N_total_all",format="df") |> select(starts_with("N_total_all"))
  hist <- tibble(Scenario="Historical",Year=years,mean=colMeans(Ntot),
                 lo=apply(Ntot,2,quantile,CI_LO),hi=apply(Ntot,2,quantile,CI_HI))
...
create_effect_plots_v3.2 <- function(fit, save=TRUE, prefix="IPM_v3.2") {
  ...
  make_plot <- function(param, xlab, title, xr=seq(-2,2,length.out=100), ylims=NULL) {
    ...
    sm <- df |> group_by(x) |>
      summarise(mean=mean(survival),lo=quantile(survival,CI_LO),
                hi=quantile(survival,CI_HI),.groups="drop")
    ...
    p <- ggplot(sm,aes(x=x)) +
      ...
      labs(x=xlab,y="Pup Survival",title=title) + theme_seal()
  ...
  comp_df <- map_dfr(names(res), function(sn)
    bind_rows(
      tibble(Scenario=sn,Year=pyrs,Synchrony="Asynchronous (current)",
             ...
             lo=apply(res[[sn]]$async,2,quantile,CI_LO),
             hi=apply(res[[sn]]$async,2,quantile,CI_HI)),
      tibble(Scenario=sn,Year=pyrs,Synchrony="Synchronous (hypothetical)",
             mean=colMeans(res[[sn]]$sync),
             lo=apply(res[[sn]]$sync,2,quantile,CI_LO),
             hi=apply(res[[sn]]$sync,2,quantile,CI_HI))
    ))
