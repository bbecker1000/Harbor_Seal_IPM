...
# ----------------------------------------------------------------------------
# Combined from three separate scripts:
#   06_ipm_plots.R  (v3.2 base — all v3.2 functions)
#   06_ipm_plots_v3.3_additions.R (rho_pup additions + create_molt_pup_plots)
#   06a_amj_molt_plot  (standalone AMJ -> molt detection plot, now a function)
#
# v3.3 vs v3.2 changes:
#   - rho_pup added to diagnostics, trace plots, summary table, recovery
#   - create_molt_pup_plots_v3.3(): 4 plots for pup molt decomposition
#   - create_amj_molt_plot_v3.3():  AMJ -> molt detection curve (was 06a)
#   - run_all_plots_v3.3() calls all three new functions
#
# Usage:
#   source("Code/05_ipm_model.R"); source("Code/06_ipm_plots.R")
#   out <- load_seal_results("IPM_v3.3_real")
#   run_all_plots_v3.3(out$fit, out$sim_data, prefix="IPM_v3.3_real")
# ============================================================================
...
# ── Load helper ───────────────────────────────────────────────────────────────
load_seal_results <- function(prefix="IPM_v3.3_real", fit_path=NULL,
                              input_data_path=NULL, years=1997:2025, T_proj=10) {
  if (is.null(fit_path))
  ...
  if (!is.null(input_data_path) && file.exists(input_data_path)) {
    ...
    sim_data <- list(stan_data=inp$stan_data, site_names=c("BL","DE","DP","PRH","TB","TP"),
    ...
    sim_data <- list(
      ...
      site_names=c("BL","DE","DP","PRH","TB","TP"), years=years[seq_len(T_val)],
      scenario_names=c("Status Quo","Warm (MOCI +1)","Cool (MOCI -1)","Warm + High Coyote"),
      true_params=NULL)
  }
...
seal_fit_summary <- function(fit, variables) {
  ...
  s <- tryCatch(fit$summary(variables=variables), error=function(e) stop(stale_msg))
  d <- tryCatch(fit$draws(variables=variables, format="matrix"), error=function(e) stop(stale_msg))
  s$q_lo <- apply(d[, s$variable, drop=FALSE], 2, quantile, CI_LO)
  s$q_hi <- apply(d[, s$variable, drop=FALSE], 2, quantile, CI_HI)
  .to_num <- function(x) if (is.list(x)) vapply(x, function(v) suppressWarnings(as.numeric(v)[1L]), numeric(1))
  else suppressWarnings(as.numeric(x))
  for (col in c("rhat","ess_bulk","ess_tail")) if (col %in% names(s)) s[[col]] <- .to_num(s[[col]])
  s
}

# ============================================================================
# ORCHESTRATOR  (v3.3 — includes AMJ molt plot)
# ============================================================================
run_all_plots_v3.3 <- function(fit, sim_data, prefix="IPM_v3.3",
  ...
  safe_run <- function(label, expr) {
    cat(sprintf("\n%s\n", label))
    tryCatch(withCallingHandlers(expr, warning=function(w) invokeRestart("muffleWarning")),
  ...
  params_candidate <- c(
    ...
    "beta_moci_ond_pup","beta_moci_jfm_pup","beta_moci_amj_molt",
  ...
  params <- if (!is.null(diag) && !is.null(diag$params)) diag$params[diag$params %in% available_vars]
  else params_candidate[params_candidate %in% available_vars]
  
  traces   <- safe_run("── Trace plots ──",        create_trace_plots_v3.3(fit, params, save=save, prefix=prefix))
  rec      <- NULL
  if (run_recovery && !is.null(sim_data$true_params))
    rec    <- safe_run("── Parameter recovery ──",  check_parameter_recovery_v3.3(fit, sim_data, save=save, prefix=prefix))
  ...
  amj_molt <- safe_run("── AMJ molt detection ──",  create_amj_molt_plot_v3.3(fit, save=save, prefix=prefix))
  ...
  molt_pup <- safe_run("── Molt pup decomp ──",     create_molt_pup_plots_v3.3(fit, sim_data, save=save, prefix=prefix))
  ...
  results <- list(diagnostics=diag, traces=traces, recovery=rec, ppc=ppc, ts=ts,
                  site_age=sa, site_panels=sasite, projections=proj,
                  effects=eff, juv_adult_effects=jveff, amj_molt=amj_molt,
                  forest=forest, decomposition=decomp, lambda_elast=le,
                  moci_collin=moci, table=tbl, molt_pup=molt_pup,
...
check_diagnostics_v3.3 <- function(fit) {
  ...
  }, error=function(e) cat("NOTE: sampler diagnostics unavailable (CSVs gone).\n"))
  params <- c("phi_pup_logit","phi_juv_base","phi_adult_F_logit","phi_adult_F_base","delta_adult",
              "fecund_primip","fecund_mature","prop_female","p_male_breed","rho_pup",
              "beta_coy[1]","beta_coy[2]","beta_coy[3]",paste0("beta_dist_surv[",1:6,"]"),
              "beta_moci_ond_fecund","beta_moci_amj_pup","beta_moci_jfm_juv","beta_moci_jfm_adult","beta_eseal_pup",
              "sigma_process","sigma_obs_adult","sigma_obs_pup","sigma_obs_molt","sigma_site")
  ...
  if (nrow(rho_row)>0)
    cat(sprintf("\nrho_pup (pup molt attendance): mean=%.3f, %s=[%.3f, %.3f]\n",
  ...
  bad <- s[!is.na(rhat_v) & rhat_v>1.05,,drop=FALSE]; low <- s[!is.na(ess_v) & ess_v<400,,drop=FALSE]
...
create_trace_plots_v3.3 <- function(fit, params, save=TRUE, prefix="IPM_v3.3") {
  ...
  list(
    ...
    errors      = grp(c("sigma_process","sigma_obs_adult","sigma_obs_pup","sigma_obs_molt",
                        "sigma_site","rho_pup"),"Trace: Error Terms + rho_pup","trace_errors"))
}

# ============================================================================
# PARAMETER RECOVERY (v3.3: rho_pup)
# ============================================================================
check_parameter_recovery_v3.3 <- function(fit, sim_data, save=TRUE, prefix="IPM_v3.3") {
  ...
  true_vals <- tibble(
    parameter = c("phi_pup_logit","phi_juv_base","phi_adult_F_logit","delta_adult",
                  "fecund_primip","fecund_mature","prop_female","p_male_breed","rho_pup",
                  "beta_coy[1]","beta_coy[2]","beta_coy[3]",paste0("beta_dist_surv[",1:6,"]"),
                  "beta_moci_ond_fecund","beta_moci_ond_pup","beta_moci_jfm_pup","beta_moci_amj_pup",
                  "beta_moci_jfm_juv","beta_moci_jfm_adult","beta_eseal_pup",
    ...
    true_value = c(tp$phi_pup_logit,tp$phi_juv_base,tp$phi_adult_F_logit,tp$delta_adult,
                   tp$fecund_primip,tp$fecund_mature,tp$prop_female,tp$p_male_breed,tp$rho_pup,
                   ...
                   tp$beta_moci_ond_fecund,tp$beta_moci_ond_pup,tp$beta_moci_jfm_pup,tp$beta_moci_amj_pup,
                   tp$beta_moci_jfm_juv,tp$beta_moci_jfm_adult,tp$beta_eseal_pup,
  ...
  cat("Recovery:", sum(rec$recovered),"/",nrow(rec),sprintf("(%.1f%%)\n",100*mean(rec$recovered)))
  rec <- rec |> mutate(
    ...
    identifiability=factor(identifiability,levels=c("Well identified","Moderately identified","Prior dominated")))
  p <- ggplot(rec,aes(x=true_value,y=mean)) +
    ...
    scale_color_manual(values=c("TRUE"=SEAL_COLS$pop,"FALSE"=SEAL_COLS$adult_f),name=paste("True in",CI_LABEL)) +
    ...
    labs(x="True Value",y=paste0("Posterior Mean (",CI_LABEL,")"),title="Parameter Recovery: IPM v3.3") +
    theme_seal()
  if (save) ggsave(paste0("Output/Plots/",prefix,"_parameter_recovery.jpeg"),p,width=30,height=22,units="cm")
...
create_ppc_plots_v3.2 <- function(fit, sim_data, save=TRUE, prefix="IPM_v3.2") {
  ...
  if (save) ggsave(paste0("Output/Plots/",prefix,"_ppc_density.jpeg"),p_comb,width=25,height=30,units="cm")
  ...
  if (save) ggsave(paste0("Output/Plots/",prefix,"_ppc_by_site.jpeg"),p_site,width=25,height=15,units="cm")
  list(density=p_comb,by_site=p_site)
}

# ============================================================================
# TIME SERIES (v3.2 — unchanged)
# ============================================================================
create_timeseries_plots_v3.2 <- function(fit, sim_data, save=TRUE, prefix="IPM_v3.2") {
  ...
  p_total <- ggplot(plot_df,aes(x=Year))+
    ...
    scale_y_continuous(limits=c(0,ceiling(max(plot_df$hi)/1000)*1000),labels=scales::comma,expand=c(0,0))+
    labs(x="Year",y="Total Population",title="Estimated Total Harbor Seal Population")+theme_seal()
  if (save) ggsave(paste0("Output/Plots/",prefix,"_total_population.jpeg"),p_total,width=25,height=15,units="cm")
  ...
  p_phipup <- ggplot(tibble(x=pup_prob),aes(x=x))+
    ...
    labs(x="Pup survival probability",y="Density",title="Posterior: Pup Survival (sex-neutral)",
         subtitle=sprintf(paste0("Median=%.3f (",CI_FMT,": %.3f-%.3f)"),
                          median(pup_prob),quantile(pup_prob,CI_LO),quantile(pup_prob,CI_HI)))+theme_seal()
  if (save) ggsave(paste0("Output/Plots/",prefix,"_phi_pup_posterior.jpeg"),p_phipup,width=20,height=12,units="cm")
  ...
  p_pmb <- ggplot(tibble(x=pmb),aes(x=x))+
    ...
    labs(x="p_male_breed",y="Density",title="Posterior: Male Haul-out Fraction (Breeding Season)",
         subtitle=sprintf(paste0("Median=%.3f (",CI_FMT,": %.3f-%.3f)"),
                          median(pmb),quantile(pmb,CI_LO),quantile(pmb,CI_HI)))+theme_seal()
  if (save) ggsave(paste0("Output/Plots/",prefix,"_p_male_breed.jpeg"),p_pmb,width=20,height=12,units="cm")
  ...
  p_phipup_t <- ggplot(mp_df,aes(x=Year))+
    ...
    labs(x="Year",y="Pup survival",title="Mean Pup Survival Over Time")+theme_seal()
  if (save) ggsave(paste0("Output/Plots/",prefix,"_phi_pup_timeseries.jpeg"),p_phipup_t,width=22,height=12,units="cm")
  list(total=p_total,phi_pup=p_phipup,p_male_breed=p_pmb,phi_pup_time=p_phipup_t)
}

# ============================================================================
# SITE-BY-AGE TIME SERIES (v3.2 — unchanged)
# ============================================================================
create_site_age_timeseries_v3.2 <- function(fit, sim_data, save=TRUE, prefix="IPM_v3.2") {
  ...
  pull_st <- function(var) {
    ...
    map_dfr(1:S,function(s) map_dfr(1:T,function(t) {
      ...
      tibble(Site=site_names[s],Year=years[t],mean=mean(d[,cn]),
             lo=as.numeric(quantile(d[,cn],CI_LO)),hi=as.numeric(quantile(d[,cn],CI_HI)))
    }))
  }
  all_sum <- bind_rows(pull_st("N_pup")|>mutate(Age_Class="Pup"),
  ...
  p <- ggplot(all_sum,aes(x=Year,y=mean,color=Site,fill=Site))+
    geom_ribbon(aes(ymin=lo,ymax=hi),alpha=0.15,color=NA)+geom_line(linewidth=1)+
    facet_wrap(~Age_Class,scales="free_y",ncol=1)+expand_limits(y=0)+
    scale_y_continuous(labels=scales::comma)+
    labs(x="Year",y="Population Size",title="Population by Age Class")+theme_seal()
  if (save) ggsave(paste0("Output/Plots/",prefix,"_age_class_timeseries.jpeg"),p,width=30,height=35,units="cm")
...
create_site_panels_v3.2 <- function(fit, sim_data, save=TRUE, prefix="IPM_v3.2") {
  ...
  all_df <- bind_rows(pull_st("N_pup","Pup"),pull_st("N_juv_total","Juvenile"),pull_st("N_adult_total","Adult"))|>
    mutate(Age_Class=factor(Age_Class,levels=c("Pup","Juvenile","Adult")),Site=factor(Site,levels=site_names))
  p <- ggplot(all_df,aes(x=Year,y=mean,colour=Age_Class,fill=Age_Class))+
    ...
    scale_colour_manual(values=age_cols,guide="none")+scale_fill_manual(values=age_cols,guide="none")+
    labs(x="Year",y="Population Size",title="Harbor Seal Population by Site and Age Class",
         subtitle=paste0("Posterior mean +/- ",CI_LABEL))+
    theme_seal(base_size=14)+theme(axis.text.x=element_text(angle=45,hjust=1,size=8),
                                   strip.text.y=element_text(size=10,face="bold"),panel.spacing=unit(0.4,"lines"))
  if (save) ggsave(paste0("Output/Plots/",prefix,"_site_age_panels.jpeg"),p,width=36,height=6*S,units="cm",dpi=200)
  list(plot=p,data=all_df)
}

# ============================================================================
# PROJECTIONS (v3.2 — unchanged)
# ============================================================================
create_projection_plots_v3.2 <- function(fit, sim_data, save=TRUE, prefix="IPM_v3.2") {
  ...
  p <- ggplot()+
    ...
    scale_y_continuous(limits=c(0,ceiling(max(full$hi,na.rm=TRUE)/1000)*1000),labels=scales::comma,expand=c(0,0))+
    labs(x="Year",y="Total Population",title="10-Year Projections",
         subtitle=paste0("Bands = ",CI_LABEL,"; dashed = projection start"))+theme_seal()
  if (save) ggsave(paste0("Output/Plots/",prefix,"_projections.jpeg"),p,width=30,height=20,units="cm")
  list(projection=p,data=proj_df)
}

# ============================================================================
# COVARIATE EFFECT ENGINE (v3.2 — unchanged)
# ============================================================================
.make_effect_engine <- function(all_draws) {
  function(param, xlab, title, stage="pup", xr=seq(-2,2,length.out=100), ylims=NULL) {
    ...
    if (length(beta_v)==0L) return(list(plot=ggplot()+labs(title=paste("MISSING:",param))+theme_seal(),yr=c(0,1)))
...
.get_draws <- function(fit) tryCatch(fit$draws(format="draws_df"),
                                     error=function(e) tryCatch(fit$draws(format="matrix"),
                                                                error=function(e2) stop("Cannot extract draws; pass out$fit.")))
...
create_effect_plots_v3.2 <- function(fit, save=TRUE, prefix="IPM_v3.2") {
  ...
  coy <- lapply(1:3,function(i) mk(paste0("beta_coy[",i,"]"),"Coyote (SD)",paste0("(",c("BL","DE","DP")[i],")")))
  ...
  p_mci <- wrap_plots(lapply(mci_specs,function(x)
    mk(x[1],x[2],x[3],stage=x[4],ylims=ylm)$plot),ncol=3)+
...
create_juv_adult_effect_plots_v3.2 <- function(fit, save=TRUE, prefix="IPM_v3.2") {
  ...
  yall <- c(max(0,min(c(j1$yr,a1$yr,a2$yr))-0.02),min(1,max(c(j1$yr,a1$yr,a2$yr))+0.02))
...
# ============================================================================
# AMJ -> MOLT DETECTION PLOT  (v3.3 — NEW)
# ----------------------------------------------------------------------------
# Standalone function wrapping the 06a_amj_molt_plot code.
# Shows how spring MOCI (AMJ) affects molt-survey detection probability —
# the positive beta reflects behavioural haul-out response (more food =>
# more time hauling out during the survey window).
# ============================================================================
create_amj_molt_plot_v3.3 <- function(fit, save=TRUE, prefix="IPM_v3.3") {
  draws   <- fit$draws(format="df")
  ...
  # Build curve summary — row per x-value, no aggregate()
  sm <- do.call(rbind, lapply(xr, function(x) {
    vals <- plogis(logit_v[idx] + beta_v[idx] * x)
    data.frame(cov_val=x, mn=mean(vals),
               lo=as.numeric(quantile(vals, CI_LO)),
               hi=as.numeric(quantile(vals, CI_HI)))
  }))
  
  base_det <- mean(plogis(logit_v))
  beta_med  <- median(beta_v)
  beta_lo   <- as.numeric(quantile(beta_v, CI_LO))
  beta_hi   <- as.numeric(quantile(beta_v, CI_HI))
  
  p4e <- ggplot(sm, aes(x=cov_val)) +
    geom_ribbon(aes(ymin=lo, ymax=hi), alpha=0.20, fill="blue") +
    geom_line(aes(y=mn), linewidth=1.2, colour="blue3") +
    geom_hline(yintercept=base_det, linetype=2, colour="gray50") +
    geom_vline(xintercept=0,         linetype=2, colour="gray50") +
    annotate("text", x=-1.8, y=base_det, vjust=-0.5, hjust=0, size=3.5, colour="gray40",
             label=sprintf("Baseline p = %.2f", base_det)) +
    labs(x="MOCI Spring AMJ (SD units)",
         y="Molt detection probability",
         title="AMJ \u2192 Molt Detection Probability",
         subtitle=sprintf("beta_MOCI_AMJ_molt: median=%.3f, %s [%.3f, %.3f] (positive = warm spring increases haul-out)",
                          beta_med, CI_LABEL, beta_lo, beta_hi)) +
    coord_cartesian(ylim=c(0, 1)) +
    theme_seal()
  
  if (save) ggsave(paste0("Output/Plots/", prefix, "_effects_amj_molt.jpeg"),
                   p4e, width=20, height=12, units="cm")
  p4e
}

# ============================================================================
# FOREST PLOT (v3.2 — unchanged)
# ============================================================================
create_forest_plot_v3.2 <- function(fit, save=TRUE, prefix="IPM_v3.2") {
  params <- tribble(
    ...
    "detect_breed_logit",   "Detect breed (logit)",         "Detection",
    "detect_molt_logit",    "Detect molt (logit)",          "Detection",
  ...
  post <- map_dfr(seq_len(nrow(params)),function(i) {
    v <- tryCatch(as.numeric(posterior::extract_variable(all_draws,params$variable[i])),error=function(e) numeric(0))
  ...
  df <- post|>left_join(params,by="variable")|>
    mutate(label=factor(label,levels=rev(params$label)),group=factor(group,levels=names(grp_cols)),sig=(lo89>0)|(hi89<0))
  prior_specs <- tribble(
    ~variable,~prior_mean,~prior_sd,
    "beta_moci_ond_fecund",-0.15,0.20,"beta_moci_ond_pup",-0.15,0.20,
    "beta_moci_jfm_pup",-0.15,0.20,"beta_moci_amj_pup",-0.15,0.20,
    "beta_moci_jfm_juv",-0.15,0.15,"beta_moci_jfm_adult",-0.10,0.12,
    "beta_moci_amj_molt",0.05,0.15,"detect_breed_logit",1.20,0.50,
    "detect_molt_logit",0.75,0.50,"beta_coy[1]",-0.20,0.20,
    "beta_coy[2]",-0.20,0.20,"beta_coy[3]",-0.20,0.20,
    "beta_dist_surv[1]",-0.15,0.20,"beta_dist_surv[2]",-0.15,0.20,
    "beta_dist_surv[3]",-0.15,0.20,"beta_dist_surv[4]",-0.15,0.20,
    "beta_dist_surv[5]",-0.15,0.20,"beta_dist_surv[6]",-0.15,0.20,
    "beta_eseal_pup",0.10,0.20)
  prior_df <- prior_specs|>
    mutate(prior_lo89=prior_mean+qnorm(CI_LO)*prior_sd,prior_hi89=prior_mean+qnorm(CI_HI)*prior_sd,
           prior_lo50=prior_mean+qnorm(0.25)*prior_sd,prior_hi50=prior_mean+qnorm(0.75)*prior_sd)|>
    left_join(params|>select(variable,label,group),by="variable")|>filter(!is.na(label))|>
    mutate(label=factor(label,levels=rev(params$label)),group=factor(group,levels=names(grp_cols)))
  p <- ggplot(df,aes(y=label,colour=group))+
    geom_linerange(data=prior_df,aes(y=label,xmin=prior_lo89,xmax=prior_hi89),colour="grey72",
                   linewidth=0.6,alpha=0.85,position=position_nudge(y=0.25),inherit.aes=FALSE)+
    geom_linerange(data=prior_df,aes(y=label,xmin=prior_lo50,xmax=prior_hi50),colour="grey58",
                   linewidth=2.0,alpha=0.75,position=position_nudge(y=0.25),inherit.aes=FALSE)+
    geom_point(data=prior_df,aes(y=label,x=prior_mean),colour="grey48",size=2.2,shape=1,
    ...
    labs(x="Coefficient (logit scale)",y=NULL,title="Covariate Effects Forest Plot",
         subtitle=paste0("Posterior: thick=50% CrI, thin=",CI_LABEL,"; grey=prior"))+
  ...
  if (save) ggsave(paste0("Output/Plots/",prefix,"_forest_plot.jpeg"),p,width=28,height=26,units="cm",dpi=200)
  list(plot=p,data=df)
}

# ============================================================================
# SUMMARY TABLE (v3.3: rho_pup)
# ============================================================================
create_summary_table_v3.3 <- function(fit, save=TRUE, prefix="IPM_v3.3") {
  params <- c("phi_pup_logit","phi_juv_base","phi_adult_F_logit","phi_adult_F_base","delta_adult",
              "fecund_primip","fecund_mature","prop_female","avg_fecundity","p_male_breed","rho_pup",
              "beta_coy[1]","beta_coy[2]","beta_coy[3]",paste0("beta_dist_surv[",1:6,"]"),
              "beta_moci_ond_fecund","beta_moci_amj_pup","beta_moci_jfm_juv","beta_moci_jfm_adult",
  ...
  tbl <- seal_fit_summary(fit,params)|>bind_rows(pup_base)|>
    mutate(Estimate=sprintf("%.3f (%.3f, %.3f)",mean,q_lo,q_hi),
           Category=case_when(
             ...
             variable=="rho_pup" ~ "Molt attendance (v3.3)",
...
# ============================================================================
# KEY RESULTS (v3.2 — unchanged but prints rho_pup if present)
# ============================================================================
save_model_output_v3.2 <- function(fit, prefix="IPM_v3.2") {
  ...
  cat("\n=== HARBOR SEAL IPM v3.3 — KEY RESULTS ===\n")
  ...
  tryCatch({
    rho <- seal_fit_summary(fit,"rho_pup")
    cat(sprintf("rho_pup (molt att):  %.3f (%.3f-%.3f)\n",rho$mean,rho$q_lo,rho$q_hi))
  ...
  tryCatch({
    ...
    cat(sprintf("\nDetect breed: %.3f -> p=%.2f\nDetect molt:  %.3f -> p=%.2f\n",
                dtb$mean[1],plogis(dtb$mean[1]),dtb$mean[2],plogis(dtb$mean[2])))
  }, error=function(e) NULL)
}

# ============================================================================
# COVARIATE DECOMPOSITION (v3.2 — unchanged)
# ============================================================================
create_covariate_decomposition_plots_v3.2 <- function(fit, sim_data, save=TRUE, prefix="IPM_v3.2") {
  ...
  sdat <- if (!is.null(sim_data$stan_data$moci_amj)) sim_data$stan_data else sim_data
  bvars <- c(paste0("beta_coy[",1:3,"]"),paste0("beta_dist_surv[",1:S,"]"),
             "beta_eseal_pup","beta_moci_amj_pup","beta_moci_ond_fecund","beta_moci_jfm_juv","beta_moci_jfm_adult")
  bsum <- fit$summary(variables=bvars)
  gm  <- function(v){r<-bsum$mean[bsum$variable==v];if(length(r)==0L) 0 else r}
  gmo <- function(v) tryCatch({x<-fit$summary(variables=v)$mean;if(length(x)==0L) 0 else x},error=function(e) 0)
  b_coy<-sapply(1:3,function(k)gm(paste0("beta_coy[",k,"]")))
  b_dst<-sapply(1:S,function(k)gm(paste0("beta_dist_surv[",k,"]")))
  b_es<-gm("beta_eseal_pup"); b_amj<-gm("beta_moci_amj_pup")
  b_ond_pup<-gmo("beta_moci_ond_pup"); b_jfm_pup<-gmo("beta_moci_jfm_pup")
  ...
  contrib <- map_dfr(1:S,function(si) map_dfr(seq_along(years),function(t) {
    ...
    tibble(Site=site_names[si],Year=years[t],
           `Coyote (t-1)`=coy,`Disturbance (t-1)`=b_dst[si]*sdat$disturbance[si,tb],
           `Elephant seal (t-1)`=if(has_es[si]>0) b_es*sdat$elephant_seal[si,tb] else 0,
  ...
  cov_cols <- c(`Coyote (t-1)`="#B2182B",`Disturbance (t-1)`="#8C510A",`Elephant seal (t-1)`="#762A83",
                `MOCI AMJ -> pup (t-1)`="#4DAC26",`MOCI OND -> pup (t)`="#B8E186",`MOCI JFM -> pup (t)`="#74C476")
  ...
  pup_long <- contrib|>select(Site,Year,all_of(pup_vars))|>
    ...
    filter(!is.na(Effect)&Effect!=0)|>mutate(Covariate=factor(Covariate,levels=pup_vars))
  p_pup <- ggplot(pup_long,aes(x=Year,y=Effect,fill=Covariate))+
    ...
    labs(x="Year",y="Covariate effect (logit scale)",title="Covariate Contributions to Pup Survival")+
    theme_seal()+theme(axis.text.x=element_text(angle=45,hjust=1,size=9),legend.position="bottom",
                       strip.text=element_text(size=11,face="bold"),panel.spacing=unit(0.5,"lines"))
  if (save) ggsave(paste0("Output/Plots/",prefix,"_decomp_pup_survival.jpeg"),p_pup,width=32,height=30,units="cm",dpi=200)
  list(pup_survival=p_pup,data=contrib)
}

# ============================================================================
# PORTFOLIO ANALYSIS (v3.2 — unchanged)
# ============================================================================
create_portfolio_analysis_v3.2 <- function(fit, sim_data, save=TRUE, prefix="IPM_v3.2") {
  ...
  per <- cv_meta/cv_sites; lcor <- cor(t(lmat),use="pairwise.complete.obs")
  cat(sprintf("Portfolio Effect Ratio: %.3f  Mean site corr: %.3f\n",per,mean(lcor[lower.tri(lcor)])))
  ...
  p_heat <- ggplot(ldf,aes(x=Year,y=Site,fill=lambda))+
    ...
    scale_fill_gradient2(low="red3",mid="white",high="darkgreen",midpoint=1,
                         limits=c(0.7,1.3),oob=scales::squish,name="lambda")+
    geom_text(aes(label=sprintf("%.2f",lambda)),size=2.5)+
    labs(x="Year",y="Site",title="Site-Specific lambda")+
    theme_seal()+theme(axis.text.x=element_text(angle=45,hjust=1,size=8),panel.grid=element_blank())
  if (save) ggsave(paste0("Output/Plots/",prefix,"_portfolio_lambda_heatmap.jpeg"),p_heat,width=35,height=15,units="cm")
  port_summary_tbl <- tibble(
    Metric=c("Portfolio Effect Ratio","Mean between-site corr","Mean per-site lambda"),
...
create_synchrony_projections_v3.2 <- function(fit, sim_data, n_sims=2000, T_proj=10,
  ...
  scenarios <- list(list(name="Status Quo",moci=0,coyote=0),list(name="Cool (MOCI -1)",moci=-1,coyote=0),
                    list(name="Warm (MOCI +1)",moci=1,coyote=0),list(name="Warm + High Coyote",moci=1,coyote=1))
  ...
  res <- lapply(scenarios,function(sc){cat(sprintf("Projecting: %s\n",sc$name));list(async=run_proj(FALSE,sc),sync=run_proj(TRUE,sc))})
  ...
  list(cv_comparison=cv_df,raw_projections=res)
}

# ============================================================================
# LAMBDA + ELASTICITY (v3.2 — unchanged)
# ============================================================================
create_lambda_elasticity_v3.2 <- function(fit, save=TRUE, prefix="IPM_v3.2") {
  ...
  T <- ncol(Ntot)
  lambda_draws <- Ntot[,2:T,drop=FALSE]/Ntot[,1:(T-1),drop=FALSE]
  lam_med <- apply(lambda_draws,2,median)
  lam_mean<-mean(lam_med);lam_sd<-sd(lam_med);lam_min<-min(lam_med);lam_max<-max(lam_med)
  ...
  cat(sprintf("\n── Aggregate lambda ──\nMean=%.3f SD=%.3f range=[%.3f,%.3f]\nOverall-mean %s: %.3f (%.3f-%.3f)\nYrs>1: %d  Yrs<1: %d\n",
              lam_mean,lam_sd,lam_min,lam_max,CI_LABEL,mean(mean_lambda_per_draw),cri_lo,cri_hi,yrs_above,yrs_below))
  elas_of <- function(M){ev<-eigen(M);k<-which.max(Re(ev$values));lam<-Re(ev$values[k]);w<-Re(ev$vectors[,k])
  vt<-Re(eigen(t(M))$vectors[,k]);list(lambda=lam,elas=(outer(vt,w)/sum(vt*w))*(M/lam))}
  build_A <- function(phi_pup,phi_juv,phi_aF,f_avg,pf)
    matrix(c(0,0,f_avg,phi_pup*pf,(2/3)*phi_juv,0,0,(1/3)*phi_juv,phi_aF),nrow=3,byrow=TRUE,
           dimnames=list(c("pup","juv_F","adult_F"),c("pup","juv_F","adult_F")))
  A <- build_A(median(draws$phi_pup_base),median(draws$phi_juv_base),
               median(draws$phi_adult_F_base),median(draws$avg_fecundity),median(draws$prop_female))
  ...
  eps <- c(adult_survival=elas["adult_F","adult_F"],
           ...
           pup_survival=elas["juv_F","pup"],fecundity=elas["pup","adult_F"])
  cat(sprintf("\n── Asymptotic lambda + elasticity ──\nlambda_det=%.4f\n",lambda_det))
  ...
  eps_draws <- t(vapply(idx,function(i){
    el<-elas_of(build_A(draws$phi_pup_base[i],draws$phi_juv_base[i],
                        draws$phi_adult_F_base[i],draws$avg_fecundity[i],draws$prop_female[i]))$elas
    c(adult_survival=el["adult_F","adult_F"],juv_survival=el["adult_F","juv_F"]+el["juv_F","juv_F"],
      pup_survival=el["juv_F","pup"],fecundity=el["pup","adult_F"])},numeric(4)))
  elas_tbl <- tibble(vital_rate=names(eps),elasticity=as.numeric(eps),
                     cri_lo=apply(eps_draws,2,quantile,CI_LO),
                     cri_hi=apply(eps_draws,2,quantile,CI_HI))|>arrange(desc(elasticity))
  if (save){write_csv(elas_tbl,paste0("Output/",prefix,"_elasticity_table.csv"))}
  p_elas <- ggplot(elas_tbl,aes(x=reorder(vital_rate,elasticity),y=elasticity))+
    ...
    labs(x=NULL,y="Elasticity",title="Elasticity of lambda to Vital Rates",
         subtitle=sprintf("Asymptotic lambda=%.3f; bars=%s",lambda_det,CI_LABEL))+theme_seal()
  if (save) ggsave(paste0("Output/Plots/",prefix,"_elasticity.jpeg"),p_elas,width=22,height=14,units="cm")
  lam_df <- tibble(t=1:(T-1),median=lam_med,
                   lo=apply(lambda_draws,2,quantile,CI_LO),hi=apply(lambda_draws,2,quantile,CI_HI))
  p_lam <- ggplot(lam_df,aes(x=t,y=median))+
    ...
    labs(x="Transition",y=expression(lambda),title="Annual Aggregate Population Growth Rate",
         subtitle=sprintf("Mean %.3f; %d/%d transitions > 1.0; bands=%s",lam_mean,yrs_above,T-1,CI_LABEL))+theme_seal()
  if (save) ggsave(paste0("Output/Plots/",prefix,"_lambda_annual.jpeg"),p_lam,width=24,height=12,units="cm")
  list(elasticity_table=elas_tbl,lambda_det=lambda_det,A=A,elasticity_plot=p_elas,lambda_plot=p_lam)
}

# ============================================================================
# MOCI COLLINEARITY (v3.2 — unchanged)
# ============================================================================
create_moci_collinearity_v3.2 <- function(fit, sim_data, save=TRUE, prefix="IPM_v3.2") {
  ...
  if (is.null(sdat$moci_jfm)) stop("MOCI series not found.")
  Tn <- length(sdat$moci_jfm)
  moci_df <- data.frame(jfm_t=sdat$moci_jfm,amj_t=sdat$moci_amj,
                        amj_tm1=c(NA,sdat$moci_amj[1:(Tn-1)]),ond_t=sdat$moci_ond,
                        ond_tm1=c(NA,sdat$moci_ond[1:(Tn-1)]))
  emp_cor <- cor(moci_df,use="pairwise.complete.obs")
  cat("\n── Empirical MOCI correlation ──\n"); print(round(emp_cor,3))
  cat(sprintf("JFM(t) vs OND(t): r=%.3f\n",emp_cor["jfm_t","ond_t"]))
  beta_vars <- c("beta_moci_jfm_pup","beta_moci_amj_pup","beta_moci_ond_pup",
                 "beta_moci_jfm_juv","beta_moci_jfm_adult","beta_moci_ond_fecund","beta_moci_amj_molt")
  beta_draws <- fit$draws(variables=beta_vars,format="df")|>dplyr::select(dplyr::all_of(beta_vars))
  post_cor <- cor(beta_draws)
  pup_sum <- beta_draws$beta_moci_jfm_pup+beta_draws$beta_moci_amj_pup+beta_draws$beta_moci_ond_pup
  agg <- tibble(quantity="Aggregate pup-survival MOCI",median=median(pup_sum),
                cri_lo=as.numeric(quantile(pup_sum,CI_LO)),cri_hi=as.numeric(quantile(pup_sum,CI_HI)),
                excludes_zero=(quantile(pup_sum,CI_LO)>0)|(quantile(pup_sum,CI_HI)<0))
  cat(sprintf("\nAggregate pup MOCI: median=%.3f %s [%.3f,%.3f] excludes0=%s\n",
  ...
  list(empirical_corr=emp_cor,posterior_corr=post_cor,aggregate_pup=agg,pup_sum_draws=pup_sum)
}

# ============================================================================
# MOLT PUP DECOMPOSITION (v3.3 — NEW)
# ============================================================================
create_molt_pup_plots_v3.3 <- function(fit, sim_data, save=TRUE, prefix="IPM_v3.3") {
  years <- sim_data$years; site_names <- sim_data$site_names
  S <- length(site_names); T <- length(years)
  rho_draws <- tryCatch(fit$draws(variables="rho_pup",format="df")$rho_pup,
                        error=function(e){warning("rho_pup not in fit (v3.2 fit?)");NULL})
  if (is.null(rho_draws)){cat("  Skipping molt_pup plots: rho_pup not found.\n");return(NULL)}
  prior_rho <- rbeta(length(rho_draws),2,5)
  dens_df <- bind_rows(tibble(value=rho_draws,Distribution="Posterior"),
  ...
  p_rho <- ggplot(dens_df,aes(x=value,fill=Distribution,colour=Distribution))+
    ...
    geom_vline(xintercept=median(rho_draws),linetype="dashed",colour=SEAL_COLS$molt,linewidth=0.9)+
    ...
    annotate("text",x=median(rho_draws),y=Inf,hjust=0,vjust=1.4,size=3.8,colour=SEAL_COLS$molt,
             label=sprintf(" Median %.2f\n %s [%.2f, %.2f]",median(rho_draws),CI_LABEL,
                           quantile(rho_draws,CI_LO),quantile(rho_draws,CI_HI)))+
    labs(x=expression(rho[pup]~"(pup molt attendance fraction)"),y="Density",
         title=expression("Posterior vs Prior: "*rho[pup]),
         subtitle="Fraction of year-class pups hauled out during peak molt count")+
    theme_seal()+theme(legend.position="top")
  pim_all <- tryCatch(fit$draws(variables="N_pup_in_molt_all",format="matrix"),error=function(e) NULL)
  p_pim_ts <- NULL
  if (!is.null(pim_all)){
    pim_df <- tibble(Year=years,mean=colMeans(pim_all),
    ...
    p_pim_ts <- ggplot(pim_df,aes(x=Year))+
      geom_ribbon(aes(ymin=lo,ymax=hi),alpha=0.25,fill=SEAL_COLS$pup)+
      geom_line(aes(y=mean),linewidth=1.2,colour=SEAL_COLS$pup)+expand_limits(y=0)+
      scale_y_continuous(labels=scales::comma)+
      labs(x="Year",y="Estimated pups in molt count (all sites)",
           title="Estimated Pup Contribution to Peak Molt Counts")+theme_seal()
  }
  pim_site <- tryCatch(fit$draws(variables="N_pup_in_molt",format="matrix"),error=function(e) NULL)
  molt_mat <- tryCatch(fit$draws(variables="N_molt_true",format="matrix"),error=function(e) NULL)
  p_stack <- p_pct <- NULL
  if (!is.null(pim_site)&&!is.null(molt_mat)){
    stack_df <- map_dfr(1:S,function(s) map_dfr(1:T,function(t){
      ...
      tibble(Site=site_names[s],Year=years[t],N_pup_in_molt=pim_m,
             N_nonjuv_adult=molt_m-pim_m,N_molt_total=molt_m,pup_pct=100*pim_m/molt_m)
    }))|>mutate(Site=factor(Site,levels=site_names))
    long_df <- stack_df|>select(Site,Year,`Pup (rho_pup x N_pup)`=N_pup_in_molt,`Juv + Adult`=N_nonjuv_adult)|>
      pivot_longer(cols=c("Pup (rho_pup x N_pup)","Juv + Adult"),names_to="Component",values_to="Count")|>
    ...
    p_stack <- ggplot(long_df,aes(x=Year,y=Count,fill=Component))+
      geom_area(position="stack",alpha=0.80)+facet_wrap(~Site,ncol=2,scales="free_y")+
      scale_fill_manual(values=c("Juv + Adult"=SEAL_COLS$molt,"Pup (rho_pup x N_pup)"=SEAL_COLS$pup))+
      scale_y_continuous(labels=scales::comma)+
      labs(x="Year",y="Estimated N in molt count",title="Molt Count Decomposition: Pup vs Juv+Adult",
           subtitle="Posterior mean; pup component = rho_pup x N_pup",fill="Component")+
      theme_seal(base_size=13)+theme(strip.text=element_text(face="bold"),legend.position="bottom",
                                     axis.text.x=element_text(angle=45,hjust=1,size=8))
    pct_df <- stack_df|>select(Site,Year,pup_pct)|>mutate(Site=factor(Site,levels=site_names))
    p_pct <- ggplot(pct_df,aes(x=Year,y=Site,fill=pup_pct))+
      geom_tile(colour="white",linewidth=0.4)+
      geom_text(aes(label=sprintf("%.0f%%",pup_pct)),size=2.8,colour="grey10")+
      scale_fill_gradient(low="white",high=SEAL_COLS$pup,limits=c(0,NA),name="Pup %\nof molt")+
      labs(x="Year",y=NULL,title="Estimated Pup % of Peak Molt Count by Site")+
      theme_seal(base_size=13)+theme(axis.text.x=element_text(angle=45,hjust=1,size=8),panel.grid=element_blank())
  }
  if (save){
    ggsave(paste0("Output/Plots/",prefix,"_rho_pup_posterior.jpeg"),p_rho,width=20,height=13,units="cm")
    if (!is.null(p_pim_ts)) ggsave(paste0("Output/Plots/",prefix,"_pup_in_molt_timeseries.jpeg"),p_pim_ts,width=24,height=13,units="cm")
    if (!is.null(p_stack))  ggsave(paste0("Output/Plots/",prefix,"_molt_decomp_stacked.jpeg"),p_stack,width=34,height=28,units="cm",dpi=200)
    if (!is.null(p_pct))    ggsave(paste0("Output/Plots/",prefix,"_pup_pct_molt_heatmap.jpeg"),p_pct,width=34,height=14,units="cm",dpi=200)
  }
  cat(sprintf("\nrho_pup: median=%.3f (89%% CrI: %.3f, %.3f)\n",
              median(rho_draws),quantile(rho_draws,CI_LO),quantile(rho_draws,CI_HI)))
  list(rho_pup_posterior=p_rho,pup_in_molt_ts=p_pim_ts,molt_stacked=p_stack,pup_pct_heatmap=p_pct)
}

cat("\n06_ipm_plots.R (v3.3) loaded — all plotting functions ready.\n")
cat("Usage: run_all_plots_v3.3(out$fit, out$sim_data, prefix=\"IPM_v3.3_real\")\n")
